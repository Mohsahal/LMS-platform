...
const app = express();
app.set("trust proxy", 1); // for Render or behind proxy
...
app.use(express.static(path.join(__dirname, '..', 'client', 'dist')));
app.get('*', (req, res, next) => {
  const apiPrefixes = ['/secure','/media','/instructor/course','/student','/notify'];
  const isApi = apiPrefixes.some(p => req.path === p || req.path.startsWith(p + '/'))
    || req.path === '/csrf-token' || req.path === '/health' || req.path === '/favicon.ico';
...
app.use((err, req, res, next) => {
  console.error("Global error:", err.message, "Path:", req.path);

  if (err.name === 'ValidationError') return res.status(400).json({ success: false, message: "Validation error", details: err.message });
  if (err.name === 'CastError') return res.status(400).json({ success: false, message: "Invalid ID format", details: err.message });
  if (err.name === 'UnauthorizedError') return res.status(401).json({ success: false, message: "Authentication required", details: err.message });
